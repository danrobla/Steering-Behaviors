...
//------------------------------------------------------------------------
void SteeringBehavior::changeMaxSteeringForce(double f)
{
    m_pVehicle->SetMaxForce(f);
}

void SteeringBehavior::changeMaxSpeed(double f)
{
    m_pVehicle->SetMaxSpeed(f);
}

void SteeringBehavior::changeWanderJitter(double w)
{
...
void SteeringBehavior::changeWanderDistance(double w)
{
    if (On(wander) && m_pVehicle->World()->RenderWanderCircle())
    {
        m_dWanderDistance = w;
...
void SteeringBehavior::changeWanderRadius(double w)
{
    if (On(wander) && m_pVehicle->World()->RenderWanderCircle())
    {
        m_dWanderRadius = w;
...
void SteeringBehavior::changeSeparationWeight(double w)
{
    if(On(separation))
    {
        m_dWeightSeparation = w;
    }
}

void SteeringBehavior::changeAlignmentWeight(double w)
{
    if(On(allignment))
    {
        m_dWeightAlignment = w;
    }
}

void SteeringBehavior::changeCohesionWeight(double w)
{
    if(On(cohesion))
    {
        m_dWeightCohesion = w;
...
//        if (KEYDOWN(Qt::Key_D)){m_dWaypointSeekDistSq += 1.0;}
//        if (KEYDOWN(Qt::Key_C)){m_dWaypointSeekDistSq -= 1.0; Clamp(m_dWaypointSeekDistSq, 0.0f, 400.0f);}
//    }
//}
